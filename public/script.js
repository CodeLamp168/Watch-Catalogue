...
async function searchMedia(searchTerm, mediaType) {
    const searchUrl = `${config.API_BASE}search/${mediaType}?api_key=${config.API_KEY}&page=1&query=${searchTerm}`;
...
function addGenreClickListener(genreList, mediaType, getMediaFunction) {
    genreList.forEach(genre => {
        genre.addEventListener('click', () => {
            ...
            const apiEndpoint = `${config.API_BASE}discover/${mediaType}?api_key=${config.API_KEY}&with_genres=${genreValue}`;
